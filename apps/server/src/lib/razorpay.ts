...
export function razorpay() {
  if (!env.razorpay.enabled) {
    throw new ApiError(
      503,
      'Online payment is not switched on yet. Use the QR transfer option below.',
...
export type RazorpayOrder = { id: string; amount: number | string; currency: string };
...
export async function createOrder(params: {
  ...
  try {
    ...
    const detail = error as { statusCode?: number; error?: { description?: string }; message?: string };
    throw new ApiError(
      detail.statusCode === 401 ? 502 : 502,
      detail.error?.description ?? detail.message ?? 'Could not reach Razorpay.',
      'RAZORPAY_ERROR'
    );
  }
}
