...
async function assertCourseAccess(userId: string, role: string, courseId: string) {
  ...
  if (!enrollment?.isActive) throw forbidden('Activate this course to see the community wall.');
}
...
communityRouter.get(
  ...
  route(async (req, res) => {
    ...
    if (!courseId) throw badRequest('Pick a course first.');
    ...
    if (classmates.length > 0) {
      await prisma.notification.createMany({
        data: classmates.map(row => ({
          ...
          type: 'NEW_POST_IN_ENROLLED_COURSE' as const,
...
communityRouter.delete(
  ...
  route(async (req, res) => {
    ...
    if (!post) throw notFound('That post is no longer there.');
...
communityRouter.post(
  ...
  route(async (req, res) => {
    const data = parse(
      z.object({ postId: z.string().min(1), type: z.enum(REACTIONS) }),
      req.body
    );
