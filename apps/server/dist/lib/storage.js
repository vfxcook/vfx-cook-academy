...
export function assertImage(file, limit = IMAGE_LIMIT, label = 'Image') {
    if (!file.mimetype.startsWith('image/'))
        throw badRequest(`${label} must be an image file.`);
    if (file.size > limit) {
...
export function assertVideo(file) {
    if (!file.mimetype.startsWith('video/'))
        throw badRequest('Only video files are allowed here.');
}
