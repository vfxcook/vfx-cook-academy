...
adminRouter.get('/overview', route(async (_req, res) => {
    const [courses, publishedCount, lessonCount, activeStudents, pendingPayments, recentPayments, recentDoubts, completedProgress, resourceCount, studioCredits] = await Promise.all([
    ...
    const possibleProgress = Math.max(courses.reduce((sum, course) => sum + course._count.videos * course._count.enrollments, 0), 1);
...
adminRouter.get('/courses', route(async (_req, res) => {
...
adminRouter.post('/courses', upload.single('thumbnailFile'), route(async (req, res) => {
    const data = parse(courseSchema, req.body);
    const slug = slugify(data.slug);
    if (!slug)
        throw badRequest('That slug is not usable.');
    const clash = await prisma.course.findUnique({ where: { slug } });
    if (clash)
        throw conflict('A course already uses that slug.');
...
adminRouter.patch('/courses/:id', upload.single('thumbnailFile'), route(async (req, res) => {
    const data = parse(courseSchema, req.body);
    const slug = slugify(data.slug);
    if (!slug)
        throw badRequest('That slug is not usable.');
    ...
    if (clash)
        throw conflict('Another course already uses that slug.');
...
adminRouter.post('/courses/:id/publish', route(async (req, res) => {
...
adminRouter.delete('/courses/:id', route(async (req, res) => {
...
adminRouter.get('/lessons', route(async (_req, res) => {
...
adminRouter.post('/lessons', upload.single('videoFile'), route(async (req, res) => {
    ...
    if (!videoUrl)
        throw badRequest('Add a video URL or upload a file.');
    ...
    if (enrolled.length > 0) {
        await prisma.notification.createMany({
            data: enrolled.map(row => ({
                ...
                type: 'NEW_VIDEO_IN_ENROLLED_COURSE',
...
adminRouter.patch('/lessons/:id', upload.single('videoFile'), route(async (req, res) => {
    const data = parse(lessonSchema, req.body);
...
adminRouter.delete('/lessons/:id', route(async (req, res) => {
    await prisma.video.delete({ where: { id: req.params.id } });
    res.json({ ok: true });
}));
adminRouter.post('/resources', upload.single('resourceFile'), route(async (req, res) => {
    ...
    if (req.file)
        fileUrl = await saveUpload(req.file, 'resources');
    if (!fileUrl)
        throw badRequest('Upload a file or paste a link.');
...
adminRouter.delete('/resources/:id', route(async (req, res) => {
    await prisma.courseResource.delete({ where: { id: req.params.id } });
    res.json({ ok: true });
}));
adminRouter.get('/students', route(async (_req, res) => {
    ...
    res.json({
        students: students.map(student => ({
            ...
            courses: student.enrollments.map(enrollment => {
                ...
                const completed = student.videoProgress.filter(progress => progress.video.courseId === enrollment.courseId).length;
                return {
...
adminRouter.post('/students/:id/grant', route(async (req, res) => {
...
adminRouter.post('/students/:id/revoke', route(async (req, res) => {
    const { courseId } = parse(z.object({ courseId: z.string().min(1) }), req.body);
...
adminRouter.post('/payments/:id/approve', route(async (req, res) => {
    ...
    if (payment.status !== 'PENDING')
        throw conflict('That request has already been reviewed.');
...
adminRouter.post('/payments/:id/reject', route(async (req, res) => {
...
adminRouter.get('/prompts', route(async (_req, res) => {
    const prompts = await prisma.trendingPrompt.findMany({
        orderBy: [{ sortOrder: 'asc' }, { createdAt: 'desc' }]
    });
    res.json({ prompts });
}));
adminRouter.post('/prompts', upload.single('imageFile'), route(async (req, res) => {
    const data = parse(z.object({
        title: z.string().trim().min(2).max(160),
    ...
    if (!req.file)
        throw badRequest('A reference image is required.');
...
adminRouter.patch('/prompts/:id', route(async (req, res) => {
    const { isPublished } = parse(z.object({ isPublished: z.boolean() }), req.body);
...
adminRouter.delete('/prompts/:id', route(async (req, res) => {
    await prisma.trendingPrompt.delete({ where: { id: req.params.id } });
    res.json({ ok: true });
}));
adminRouter.get('/community', route(async (_req, res) => {
...
adminRouter.get('/studio', route(async (_req, res) => {
...
adminRouter.post('/studio/settings', route(async (req, res) => {
    const { key, value } = parse(z.object({ key: z.string().trim().min(1).max(80), value: z.string().max(500) }), req.body);
...
adminRouter.post('/studio/credits', route(async (req, res) => {
    const data = parse(z.object({
    ...
    if (!user)
        throw notFound('No account uses that email.');
