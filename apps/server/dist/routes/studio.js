...
import { STUDIO_BRAND_NAME, addStudioCredits, ensureStudioDefaults, getOrCreateStudioBalance, refundStudioCredits, spendStudioCredits } from '../lib/studio.js';
export const studioRouter = Router();
studioRouter.get('/overview', requireUser, route(async (req, res) => {
...
studioRouter.post('/credits/order', requireUser, route(async (req, res) => {
    ...
    if (!pack)
        throw notFound('That credit pack is not available.');
...
studioRouter.post('/credits/verify', requireUser, route(async (req, res) => {
...
studioRouter.get('/workflows', requireUser, route(async (req, res) => {
...
studioRouter.post('/workflows', requireUser, route(async (req, res) => {
    const data = parse(z.object({
...
studioRouter.post('/generate', requireUser, route(async (req, res) => {
    const data = parse(z.object({
        workflowId: z.string().optional(),
    ...
    const queued = await prisma.$transaction(async (tx) => {
        ...
        if (!model?.isEnabled)
            throw badRequest('That model is not available right now.');
        if (data.workflowId) {
...
studioRouter.get('/generations', requireUser, route(async (req, res) => {
...
studioRouter.post('/callback', route(async (req, res) => {
