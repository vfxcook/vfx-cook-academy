...
async function assertCourseAccess(userId, role, courseId) {
    ...
    if (!enrollment?.isActive)
        throw forbidden('Activate this course to join the discussion.');
}
const withAuthor = {
    user: { select: { id: true, name: true, email: true, image: true } },
    likes: { select: { userId: true } }
};
commentsRouter.post('/', route(async (req, res) => {
...
commentsRouter.post('/:id/like', route(async (req, res) => {
...
commentsRouter.delete('/:id', route(async (req, res) => {
    const comment = await prisma.timestampComment.findUnique({
        where: { id: req.params.id },
