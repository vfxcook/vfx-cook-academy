...
async function progressMap(userId, videoIds) {
    const map = new Map();
...
coursesRouter.get('/', route(async (req, res) => {
...
coursesRouter.get('/:slug', route(async (req, res) => {
...
coursesRouter.get('/:slug/lessons/:lessonId', route(async (req, res) => {
    const course = await prisma.course.findUnique({
        where: { slug: req.params.slug },
...
coursesRouter.post('/progress', requireUser, route(async (req, res) => {
...
coursesRouter.post('/license/activate', requireUser, route(async (req, res) => {
    const data = parse(z.object({ courseId: z.string().min(1), licenseCode: z.string().trim().min(4).max(20) }), req.body);
    const enrollment = await prisma.enrollment.findUnique({
        where: { userId_courseId: { userId: req.user.id, courseId: data.courseId } }
    });
    if (!enrollment?.licenseCode)
        throw badRequest('No license has been issued for this course yet.');
...
coursesRouter.get('/me/dashboard', requireUser, route(async (req, res) => {
...
coursesRouter.get('/prompts/trending', route(async (_req, res) => {
...
export function shapeComment(comment, viewerId) {
    return {
        id: comment.id,
