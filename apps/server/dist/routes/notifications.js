...
notificationsRouter.get('/', route(async (req, res) => {
...
notificationsRouter.patch('/', route(async (req, res) => {
    const data = parse(z.object({ notificationId: z.string().min(1).optional(), markAll: z.boolean().optional() }), req.body);
    if (data.markAll) {
    ...
    if (!data.notificationId)
        throw badRequest('Pick a notification to mark as read.');
    await prisma.notification.updateMany({
