...
async function loadPurchasableCourse(courseId) {
...
async function ensurePendingEnrollment(userId, courseId) {
...
paymentsRouter.post('/order', requireUser, route(async (req, res) => {
    const data = parse(z.object({ courseId: z.string().min(1), isGift: z.boolean().optional() }), req.body);
    ...
    if (!data.isGift) {
        ...
        if (enrollment?.isActive)
            throw conflict('You already have access to this course.');
    }
...
paymentsRouter.post('/verify', requireUser, route(async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (!pending)
            throw notFound('We could not find a pending payment for this course.');
...
paymentsRouter.post('/payment-link', requireUser, route(async (req, res) => {
...
paymentsRouter.post('/requests', requireUser, route(async (req, res) => {
    const data = parse(z.object({
        courseId: z.string().min(1),
    ...
    if (existing)
        throw conflict('You already have a payment under review for this course.');
...
paymentsRouter.post('/gifts/redeem', requireUser, route(async (req, res) => {
    const { code } = parse(z.object({ code: z.string().trim().min(6, 'Enter the full gift code.').max(64) }), req.body);
    const slug = await prisma.$transaction(async (tx) => {
        ...
        if (!coupon)
            throw badRequest('That gift code is not valid.');
        if (coupon.isRedeemed)
            throw conflict('That gift code has already been redeemed.');
...
export const razorpayWebhook = route(async (req, res) => {
    ...
    if (!userId || !courseId)
        return res.json({ ok: true, ignored: true, reason: 'missing notes' });
